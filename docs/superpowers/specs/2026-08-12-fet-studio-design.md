...
| 항목 | 정의 | 기본 임계 | 의미 |
|---|---|---|---|
| **0 V 오프셋** | `\|I_D(V_D=0)\| / max\|I_D\|` | > 1 % → ⚠ | 원점에서 출발하지 않음 (오프셋·누설) |
| **원점 선형성** | `\|V_D\| ≤ 전체 스윕폭의 10 %` 구간 선형 fit 의 R² | R² < 0.99 → ⚠ | S자 개형 = 컨택트 저항 / Schottky 장벽 |
...
- X: `V_G (V)`
- 좌 Y: **log**. 내부적으로는 `|I_D|` 를 그리되 **축 제목에는 절댓값 기호를 쓰지 않는다** →
  제목은 `Drain current (A)`. (photodetector-app 의 확정 규약 A2 를 그대로 따름)
- 우 Y: **linear**, 제목 `√|I_D| (A^0.5)` — 여기는 실제로 제곱근을 취한 양이므로 기호를 쓴다.
...
1. `MANUAL.md` — 저장소 루트. 텍스트라 diff 가 되고 수정 요청이 쉽다.
2. 앱 내 **[매뉴얼]** 버튼 → 확장 패널에서 동일 내용 렌더 (수식은 LaTeX)

단일 소스에서 생성한다: `MANUAL.md` 를 읽어 앱이 렌더하는 방식으로 두어 **두 벌이 어긋날 수 없게** 한다.
