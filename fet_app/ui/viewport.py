...
# 빌드엔 st.context.viewport 가 없어 실측 프로브가 항상 실패한다). k 가
# 조금이라도 크게 잡히면 apply_ui_zoom() 이 보정하지 않는 1440px 기준
# 화면에서마저 그래프 2개+gap 이 예산(~722px)을 넘칠 수 있으므로, 어림값
# 보다 살짝 보수적으로 낮춰 0.35 로 둔다. 실제 화면에서 보고 추가 튜닝이
# 필요할 수 있다.
FALLBACK_SCALE = 0.35
